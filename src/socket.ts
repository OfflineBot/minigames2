...
function empty_submited(id_list: string[]) {
    let out = []
...
function update_submited(submition_list: { id: string, ready: boolean}[], id: string) {
...
function check_all_ready(submition_list: { id: string, ready: boolean }[]) {
    let ready = true;
    for (let i = 0; i < submition_list.length; i++) {
...
export function setup_socket(io: Server, game: Game) {

    let player_count = 0;
    let game_iteration = 0;
    let player_submited: { id: string, ready: boolean }[] = []
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('game_submit', (text: string) => {
